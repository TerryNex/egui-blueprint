...
impl Default for EditorStyle {
    fn default() -> Self {
        ...
        Self { header_colors: map, use_gradient_connections: true }
    }
}
...
impl GraphEditor {
    pub fn show(&mut self, ui: &mut egui::Ui, graph: &mut BlueprintGraph, undo_stack: &mut crate::history::UndoStack) {
        ...
        ui.input(|i| {
            ...
            if i.modifiers.command {
                if i.key_pressed(egui::Key::Plus) || i.key_pressed(egui::Key::Equals) {
                     let center = if let Some(p) = i.pointer.hover_pos() { p - ui.max_rect().min } else { ui.max_rect().size() / 2.0 };
                     let delta = 1.1;
        ...
        if let Some((node_id, port_name, is_input)) = &self.connection_start {
            if let Some(pos) = pointer_pos {
                 let start_pos = self.get_port_screen_pos(ui, &graph.nodes, *node_id, port_name, *is_input, canvas_offset, &node_sizes);
        ...
        for node_id in sorted_node_ids {
            ...
            let (drag_delta, start_connect, clicked, right_clicked, clicked_port, pressed, disconnect, node_changed, delete, copy) = self.draw_node(ui, node, input_primary_pressed, input_primary_released, &graph.connections, &node_sizes);
            ...
            if drag_delta != Vec2::ZERO {
                ...
                if self.dragging_node.is_none() || self.dragging_node == Some(node.id) {
                    // When starting to drag a node that's not selected, select it first
        ...
        // Connection Cancellation
             ...
             let window_response = egui::Window::new("Add Node")
                 ...
                 .show(ui.ctx(), |ui| {
                     ...
                     let options = vec![
                         ...
                         ("Greater or Equal (>=)", crate::node_types::NodeType::GreaterThanOrEqual),
                         ("Less Than (<)", crate::node_types::NodeType::LessThan),
                         ("Less or Equal (<=)", crate::node_types::NodeType::LessThanOrEqual),
                         // Logic
                         ...
                         // Variables
                         ("Get Variable", crate::node_types::NodeType::GetVariable { name: "MyVar".into() }),
                         ("Set Variable", crate::node_types::NodeType::SetVariable { name: "MyVar".into() }),
                     ];
                    
                    let filtered_options: Vec<_> = options.into_iter()
                        .filter(|(label, _)| self.node_finder_query.is_empty() || label.to_lowercase().contains(&self.node_finder_query.to_lowercase()))
                        .collect();
                        
                    let mut activate_first = ui.input(|i| i.key_pressed(egui::Key::Enter) || i.key_pressed(egui::Key::Tab));
         ...
         if self.show_settings {
            egui::Window::new("Style Settings")
                 ...
                 .show(ui.ctx(), |ui| {
                     ui.checkbox(&mut self.style.use_gradient_connections, "Gradient Connections");
                     ui.separator();
         ...
         if copy_action && !self.selected_nodes.is_empty() {
             ...
             let connections: Vec<crate::graph::Connection> = graph.connections.iter()
                 .filter(|c| self.selected_nodes.contains(&c.from_node) && self.selected_nodes.contains(&c.to_node))
                 .cloned()
                 .collect();
         ...
         if let Some(json) = paste_content {
             if let Ok(data) = serde_json::from_str::<ClipboardData>(&json) {
                 if !data.nodes.is_empty() {
                     ...
                     let min_x = data.nodes.iter().map(|n| n.position.0).fold(f32::INFINITY, f32::min);
                     let min_y = data.nodes.iter().map(|n| n.position.1).fold(f32::INFINITY, f32::min);
                     let _max_x = data.nodes.iter().map(|n| n.position.0).fold(f32::NEG_INFINITY, f32::max);
                     let _max_y = data.nodes.iter().map(|n| n.position.1).fold(f32::NEG_INFINITY, f32::max);
                     ...
                     for conn in &data.connections {
                         if let (Some(new_from), Some(new_to)) = (id_map.get(&conn.from_node), id_map.get(&conn.to_node)) {
        ...
        match node_type {
            ...
            NodeType::Branch => (
                vec![
                    ...
                    Port { name: "Condition".into(), data_type: DataType::Boolean, default_value: VariableValue::Boolean(false) }
                ],
                vec![
                    Port { name: "True".into(), data_type: DataType::ExecutionFlow, default_value: VariableValue::None },
                    Port { name: "False".into(), data_type: DataType::ExecutionFlow, default_value: VariableValue::None }
                ]
            ),
            ...
            NodeType::ToInteger => (
                vec![Port { name: "In".into(), data_type: DataType::Custom("Any".into()), default_value: VariableValue::Integer(0) }],
                vec![Port { name: "Out".into(), data_type: DataType::Integer, default_value: VariableValue::Integer(0) }]
            ),
            NodeType::ToFloat => (
                vec![Port { name: "In".into(), data_type: DataType::Custom("Any".into()), default_value: VariableValue::Float(0.0) }],
                vec![Port { name: "Out".into(), data_type: DataType::Float, default_value: VariableValue::Float(0.0) }]
            ),
            NodeType::ToString => (
                vec![Port { name: "In".into(), data_type: DataType::Custom("Any".into()), default_value: VariableValue::String("".into()) }],
                vec![Port { name: "Out".into(), data_type: DataType::String, default_value: VariableValue::String("".into()) }]
            ),
            NodeType::Divide => (
                 vec![
                    Port { name: "A".into(), data_type: DataType::Float, default_value: VariableValue::Float(0.0) },
                    Port { name: "B".into(), data_type: DataType::Float, default_value: VariableValue::Float(1.0) }
                ],
                vec![Port { name: "Out".into(), data_type: DataType::Float, default_value: VariableValue::Float(0.0) }]
            ),
            NodeType::GetVariable { .. } => (
                vec![],
                vec![Port { name: "Value".into(), data_type: DataType::Float, default_value: VariableValue::Float(0.0) }]
            ),
            NodeType::SetVariable { .. } => (
                vec![
                    Port { name: "In".into(), data_type: DataType::ExecutionFlow, default_value: VariableValue::None },
                    Port { name: "Value".into(), data_type: DataType::Custom("Any".into()), default_value: VariableValue::Float(0.0) }
                ],
                vec![Port { name: "Next".into(), data_type: DataType::ExecutionFlow, default_value: VariableValue::None }]
            ),
            NodeType::Subtract => (
            ...
            NodeType::Equals | NodeType::NotEquals | NodeType::GreaterThan | 
            NodeType::GreaterThanOrEqual | NodeType::LessThan | NodeType::LessThanOrEqual => (
                vec![
                    Port { name: "A".into(), data_type: DataType::Float, default_value: VariableValue::Float(0.0) },
                    Port { name: "B".into(), data_type: DataType::Float, default_value: VariableValue::Float(0.0) }
                ],
                vec![Port { name: "Out".into(), data_type: DataType::Boolean, default_value: VariableValue::Boolean(false) }]
            ),
            // Logic nodes
            NodeType::And | NodeType::Or => (
                vec![
                    Port { name: "A".into(), data_type: DataType::Boolean, default_value: VariableValue::Boolean(false) },
                    Port { name: "B".into(), data_type: DataType::Boolean, default_value: VariableValue::Boolean(false) }
                ],
                vec![Port { name: "Out".into(), data_type: DataType::Boolean, default_value: VariableValue::Boolean(false) }]
            ),
            NodeType::Not => (
                vec![Port { name: "In".into(), data_type: DataType::Boolean, default_value: VariableValue::Boolean(false) }],
                vec![Port { name: "Out".into(), data_type: DataType::Boolean, default_value: VariableValue::Boolean(true) }]
            ),
            // For Loop
            NodeType::ForLoop => (
                vec![
                    Port { name: "In".into(), data_type: DataType::ExecutionFlow, default_value: VariableValue::None },
                    Port { name: "Start".into(), data_type: DataType::Integer, default_value: VariableValue::Integer(0) },
                    Port { name: "End".into(), data_type: DataType::Integer, default_value: VariableValue::Integer(10) },
                ],
                vec![
                    Port { name: "Loop".into(), data_type: DataType::ExecutionFlow, default_value: VariableValue::None },
                    Port { name: "Index".into(), data_type: DataType::Integer, default_value: VariableValue::Integer(0) },
                    Port { name: "Done".into(), data_type: DataType::ExecutionFlow, default_value: VariableValue::None },
                ]
            ),
            // While Loop
            NodeType::WhileLoop => (
                vec![
                    Port { name: "In".into(), data_type: DataType::ExecutionFlow, default_value: VariableValue::None },
                    Port { name: "Condition".into(), data_type: DataType::Boolean, default_value: VariableValue::Boolean(true) },
                ],
                vec![
                    Port { name: "Loop".into(), data_type: DataType::ExecutionFlow, default_value: VariableValue::None },
                    Port { name: "Done".into(), data_type: DataType::ExecutionFlow, default_value: VariableValue::None },
            ...
            NodeType::Delay => (
                vec![
                    Port { name: "In".into(), data_type: DataType::ExecutionFlow, default_value: VariableValue::None },
                    Port { name: "Duration (ms)".into(), data_type: DataType::Integer, default_value: VariableValue::Integer(1000) },
                ],
                vec![Port { name: "Next".into(), data_type: DataType::ExecutionFlow, default_value: VariableValue::None }]
            ),
            _ => (vec![], vec![])
    ...
    fn draw_node(&mut self, ui: &mut egui::Ui, node: &mut Node, input_primary_pressed: bool, input_primary_released: bool, connections: &[crate::graph::Connection], node_sizes: &std::collections::HashMap<Uuid, Vec2>) -> (Vec2, Option<(Uuid, String, bool)>, bool, bool, bool, bool, bool, bool, bool, bool) {
        let node_pos = Pos2::new(node.position.0, node.position.1);
        ...
        if matches!(node.node_type, crate::node_types::NodeType::GetVariable{..} | crate::node_types::NodeType::SetVariable{..}) {
        ...
        let category = match &node.node_type {
            crate::node_types::NodeType::BlueprintFunction { name } => {
                if name.starts_with("Event") { "Event" } else { "Function" }
            },
            crate::node_types::NodeType::Add | crate::node_types::NodeType::Subtract | crate::node_types::NodeType::Multiply | crate::node_types::NodeType::Divide => "Math",
            crate::node_types::NodeType::GetVariable { .. } | crate::node_types::NodeType::SetVariable { .. } => "Variable",
            _ => "Default",
        };
        
        let header_color = self.style.header_colors.get(category).unwrap_or(&Color32::GRAY);
        ...
        if self.editing_node_name == Some(node.id) {
            if let Some(ref mut name) = node.display_name {
                let edit_rect = Rect::from_center_size(header_rect.center(), Vec2::new(header_rect.width() - 10.0, 16.0 * self.zoom));
                let response = ui.scope_builder(egui::UiBuilder::new().max_rect(edit_rect), |ui| {
                    let r = ui.add(egui::TextEdit::singleline(name).hint_text("Enter name...").desired_width(edit_rect.width()));
                    r
                }).inner;
        ...
        if let crate::node_types::NodeType::GetVariable { name } | crate::node_types::NodeType::SetVariable { name } = &mut node.node_type {
            let edit_rect = Rect::from_min_size(screen_pos + Vec2::new(10.0 * self.zoom, 25.0 * self.zoom), Vec2::new(100.0 * self.zoom, 18.0 * self.zoom));
        ...
        for input in &mut node.inputs {
            ...
            if !is_connected && input.data_type != DataType::ExecutionFlow {
                ...
                let inner_changed = ui.scope_builder(egui::UiBuilder::new().max_rect(edit_rect), |ui| {
        ...
        response.context_menu(|ui| {
             if matches!(node.node_type, crate::node_types::NodeType::Add | crate::node_types::NodeType::Subtract | crate::node_types::NodeType::Multiply | crate::node_types::NodeType::Divide) {
             ...
             if matches!(node.node_type, crate::node_types::NodeType::SetVariable { .. }) {
                 ui.menu_button("Variable Type", |ui| {
                     if ui.button("Float").clicked() {
                         content_changed = true;
                         for p in &mut node.inputs { if p.name == "Value" { p.data_type = DataType::Float; p.default_value = crate::graph::VariableValue::Float(0.0); } }
                         ui.close();
                     }
                     if ui.button("Integer").clicked() {
                         content_changed = true;
                         for p in &mut node.inputs { if p.name == "Value" { p.data_type = DataType::Integer; p.default_value = crate::graph::VariableValue::Integer(0); } }
                         ui.close();
                     }
                     if ui.button("String").clicked() {
                         content_changed = true;
                         for p in &mut node.inputs { if p.name == "Value" { p.data_type = DataType::String; p.default_value = crate::graph::VariableValue::String("".into()); } }
                         ui.close();
                     }
                     if ui.button("Boolean").clicked() {
                         content_changed = true;
                         for p in &mut node.inputs { if p.name == "Value" { p.data_type = DataType::Boolean; p.default_value = crate::graph::VariableValue::Boolean(false); } }
                         ui.close();
                     }
                 });
                 ui.separator();
        ...
        (drag_delta, connect_event, clicked, right_clicked, clicked_port, pressed_node || pressed_any_port, disconnect_all, content_changed, delete_node, copy_node)
    }
    
    fn draw_connections(&mut self, ui: &egui::Ui, graph: &mut BlueprintGraph, node_sizes: &std::collections::HashMap<Uuid, Vec2>) {
        ...
        if c1_color == c2_color {
            ...
            ui.painter().add(curve);
             ...
             for i in 1..=steps {
                 ...
                 ui.painter().line_segment([prev_p, p], Stroke::new(2.0 * self.zoom, color));
    ...
    fn get_node_color(&self, graph: &BlueprintGraph, node_id: Uuid) -> Color32 {
         if let Some(node) = graph.nodes.get(&node_id) {
            ...
            *self.style.header_colors.get(category).unwrap_or(&Color32::WHITE)
         } else {
             Color32::WHITE
         }
    }
